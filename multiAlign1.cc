...
struct CorresConstraint {
    ...
    CorresConstraint(vector<int> day_pair,
                    vector<Ronin>* all_ronins
                    vector<Corres>* corres_pairs)
        : day_pair(day_pair),
          all_ronins(all_ronins),
          corres_pairs(corres_pairs) {}

    template <typename T>
    bool operator()(T const* const* params, T* residuals) const {
        vector<vector<T> > updated_x, updated_y;
        for (size_t k=0;k<day_pair.size();k++){
            int d = day_pair[k];
            ...
            pid += 3;
            updated_x[k].resize(n_velocity+1);
            updated_y[k].resize(n_velocity+1);
            updated_y[k][0] = ystart;
            updated_x[k][0] = xstart;
            for (int i=0;i<n_velocity;i++){
                xstart += mags[i]*cos(thetas[i]+gbias);
                ystart += mags[i]*sin(thetas[i]+gbias);
                updated_x[k][i+1] = xstart;
                updated_y[k][i+1] = ystart;
            }
        }
        size_t pair_size = corres_pairs.size();
        for(size_t i=0;i<pair_size;i++){
            int id0 = corres_pairs[i].id1;
            int id1 = corres_pairs[i].id2;
            residuals[i] = updated_x[0][id0] - updated_x[1][id1];
            residuals[i+pair_size] = updated_y[0][id0] - updated_y[1][id1];
        }
    	
    	return true;
    }
    static CorresCostFunction* Create(vector<int> day_pair,
    	                            vector<Ronin>* all_ronins,
                                    vector<Corres>* corres_pairs,
    	                            vector<double*>* parameter_blocks) {
    	CorresConstraint* constraint = new CorresConstraint(day1, day2, all_ronins, corres_pairs);
    	CorresCostFunction* cost_function = new CorresCostFunction(constraint);
    	// delete this new memory?
    	parameter_blocks->clear();
        for (size_t i=0;i<day_pair.size();i++){
            double* gtransfroms = (*all_ronins)[day_pair[i]].get_gtransforms();
            for(int t=0;t<3;t++){
    	...
    	cost_function->SetNumResiduals((*corres_pairs).size()*2);
...
int main(int argc, char** argv) {
    ...
    for (size_t i=0;i<folder_ids.size();i++){
        string txtname = base_path+folder_ids[i] + "/c_single_corres_align.txt";
        cout<<txtname<<endl;
    ...
    for (size_t i=0;i<folder_ids.size();i++){
        for (size_t j=0;j<folder_ids.size();j++){
            ...
            vector<Corres> corres_pairs;
            read_corres_data(txtname, corres_pairs);
            vector<int> day_pair(i,j);
            CorresConstraint::CorresCostFunction* stat_cost_function = 
                CorresConstraint::Create(
                    day_pair, &all_ronins, &corres_pairs, &parameter_blocks);
