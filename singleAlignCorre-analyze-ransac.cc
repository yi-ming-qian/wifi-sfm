...
class Ronin{
    public:
        ...
        void sample_corres_pairs(int n_reduce, int iter, string& in_path){
            ...
            string filename = in_path + "c_single_corres_sample_ransac"+to_string(iter)+".txt";
            ofstream fout(filename);
        ...
        double comp_inlier_ratio(){
            ...
            return inlier_ratio/corres_pairs.size();
        }
        void copy_bias(){
            for(int i=0;i<n_velocity;i++){
...
int main(int argc, char** argv) {
    ...
    if (n_corres>0){
        int iter_num = 20;
        int n_chosen = min(0.5*n_corres, 30.);
        if (n_chosen<=1){
            n_chosen=n_corres;
            iter_num=1;
        }
        ...
        for (int r=0;r<iter_num;r++){
            ...
            ronin.reset_bias();
            ronin.sample_corres_pairs(n_chosen, r, in_path);
            Problem problem;
            ...
            //problem.AddResidualBlock(stat_cost_function, nullptr, parameter_blocks);
            ...
            if (inlier_ratio>best_inratio){
                best_inratio=inlier_ratio;
                ronin.copy_bias();
                best_iter = r;
            }
            txtname = in_path + "c_single_corres_align_ransac"+to_string(r)+".txt";
